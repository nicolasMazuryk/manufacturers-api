...
const isTest = !!process.env.TEST_MODE;

const logger = bunyan.createLogger({
  name: 'epn-product-aggregator',
  streams: [
    {
      name: 'stdout',
...
const devlogger = {
  info(...data) {
    if (data.length === 1 && isServiceInfo(data[0])) {
      ...
      const res =
        result instanceof Exception ? inspect(result.toHash()) : result;
...
export default (isDev && !isTest ? devlogger : logger);
